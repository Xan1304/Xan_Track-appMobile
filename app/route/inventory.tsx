...
import {
  ...
  TextInput, Image, ActivityIndicator, Platform, Alert, Modal, ScrollView,
...
export default function InventoryPage() {
  ...
  async function fetchProducts() {
    try {
      setLoading(true);
      const { data: prods } = await supabase.from('danh_muc_san_pham').select('*').order('ten_san_pham', { ascending: true });
  ...
  const confirmSaveToDb = async () => {
    try {
      ...
      if (!userEmail) { Alert.alert('Lỗi', 'Phiên đăng nhập hết hạn.'); return; }
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const hasValue = stockData[item.id] !== undefined && stockData[item.id] !== '';
            return (
                ...
                <View style={styles.productInfo}>
                  ...
                  <View style={styles.inputRow}>
                    ...
                    <TextInput
                      ...
                      onChangeText={v => setStockData({ ...stockData, [item.id]: v })}
                    />
                  </View>
                </View>
              </View>
...
const styles = StyleSheet.create({
  ...
  stockInput:         { width: 54, height: 36, backgroundColor: '#f5f6f8', borderRadius: 9, textAlign: 'center', fontSize: 14, fontWeight: '600', color: '#0f172a', borderWidth: 0.5, borderColor: 'rgba(0,0,0,0.1)' },
